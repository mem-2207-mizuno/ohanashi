...
describe('AudioSenderGateway', () => {
  ...
  it('音声をアップロードできる', async () => {
    ...
    const gateway = new AudioSenderGateway('/api/upload')
    const audio = new Audio(new Blob(['test audio']), 44100, 1.2)

    // Act
    await gateway.send(audio)
  ...
  it('サーバーがエラーを返した場合は例外を投げる', async () => {
    ...
    // Act & Assert
    await expect(gateway.send(audio)).rejects.toThrowError(
  ...
  it('fetch自体が失敗した場合も例外を投げる', async () => {
    ...
    // Act & Assert
    await expect(gateway.send(audio)).rejects.toThrowError('Network Error')
  })
})